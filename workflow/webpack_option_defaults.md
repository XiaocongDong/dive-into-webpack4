# Webpack 默认配置
Webpack源代码中和默认配置相关的类有两个，它们是`WebpackOptionsDefaulter`和`OptionsDefaulter`。

以下分别介绍两个类

## lib/OptionsDefaulter
这个类是用来处理默认值的，是一个基类，后面的WebpackOptionsDefaulter是继承这个类的子类。这个类有以下两个方法：
1. set方法，用来设置某些字段的默认值或者是生成默认值的一些策略和方法。
2. process方法，根据set存储的内容，将默认值填进options对象。
...
/**
设置obj某个key的值，支持以.分隔的nested key
**/
...
class OptionsDefaulter {
	...
	process(options) {
        ...
        // 不需要当前配置生成的配置值，采取直接赋值
				case undefined:
          // 如果options没有值，才进行默认值的设置
				...
				case "call":
          // 使用默认的转换方法来转换现在的配置值
          // 例如 this.set('module', 'call', value => Object.assign({}, value))
          // 这样options里面的module值就会被浅复制
					setProperty(
						options,
						name,
					...
					break;
				case "make":
          // make，表示根据当前的options值生成这个key的配置
					if (getProperty(options, name) === undefined) {
						setProperty(options, name, this.defaults[name].call(this, options));
					}
					break;
				case "append": {
          // append, 配置值是一个数组，每次process都会将新的配置加到原来的数组里面
  ...
  /**
  这个方法在lib/WebpackOptionsDefaulter被使用
  在WebpackOptionsDefaulter的用法有以下两种：

  // 只传两个参数，表示设置这个config的默认值, 第一个参数是config的key，可以是深嵌套的，以.分隔
  this.set(entry, './src')
  this.set('optimization.splitChunks.minChunks', 1)

  // 传三个参数表示这个配置是要动态生成的， 第一个参数是key，第二个参数是生成的方式，有make，call，第三个参数就是对应的方法
  ...
  **/
	set(name, config, def) {
      ...
      // config存储生成某个config的方法
			this.config[name] = config;
		} else {
      // 如果只有两个参数，则设置name对应的配置的默认值为config
...
```
## lib/WebpackOptionsDefaulter
这个是和webpack配置直接相关的一个类，里面的内容就是调用父类OptionsDefaulter的set方法来设置webpack所有的默认值。
```javascript
/*
...
class WebpackOptionsDefaulter extends OptionsDefaulter {
	constructor() {
		...
		// 设置entry字段的默认值是'./src'
		this.set("entry", "./src");
		// devtool的默认值室友options.mode决定的，所以策略是make，然后还要提供一个生成方法
		this.set(
			"devtool",
			"make",
		...
		// webpack的默认上下文是current working directory
...
```
## 概述
Webpack将默认配置的逻辑分成了两个类，OptionsDefaulter只负责存储默认设置和生成默认配置的一些策略（make, call, append）。实际上所有的默认配置定义都放在其子类WebpackOptionsDefaulter进行。这两个类各司其职，这样后面如果出现以下需求:
...
代码都很容易进行改动，这和把所有默认配置相关的东西都耦合在一起然后一大堆if-else的语句的写法高明得多。
